...
class Resource(ABC):
    """A resource for a cloud provider.

    Resources can depend on other resources (e.g., EC2 depends on VPC, Droplet depends on SSHKey).
    The dependent resource can be provided as an argument:
      `EC2Instance(vpc=vpc_instance)` or `Droplet(sshkey=sshkey_instance)`
    If not provided, it will create a new instance of the dependency with defaults.

    Certain resources can also be retained by default (e.g., VPC, SSHKey).
