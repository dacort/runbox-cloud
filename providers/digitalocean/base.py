...
from ..aws.state import ResourceState, StateManager
...
class Resource(ABC):
    """A resource for a provider, like a Droplet or SSH Key.

    Resources can depend on other resources, like Droplet depends on SSHKey.
    The dependent resource can be provided as an argument:
      `Droplet(sshkey=sshkey_instance)`
    If not provided, it will create a new instance of the dependency with defaults.

    Certain resources can also be retained by default, like SSHKey.
